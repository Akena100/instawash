...
class CarFormState extends State<CarForm> {
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    DateTime selectedDate = _selectedDate ?? startDate;

    // If a time has been selected, compare it with the current date and time
    if (_selectedTime != null) {
      // Set the selected date's time to the selected time for comparison
    ...
    final DateTime? pickedDate = await showDatePicker(
      ...
      firstDate:
          startDate, // Restrict the selection to tomorrow or future dates
  ...
  Future<void> _selectTime(BuildContext context) async {
    ...
    TimeOfDay selectedTime = _selectedTime ?? now;

    // If a date has been selected, compare the time with the selected date
    if (_selectedDate != null) {
      ...
      if (_selectedDate!.isAtSameMomentAs(currentDateTime)) {
        if (selectedDateTime.isBefore(currentDateTime)) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('You cannot select a time in the past.')),
          );
          return;
        }
      }
